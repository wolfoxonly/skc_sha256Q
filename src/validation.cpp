...
{
	if (nHeight > 1) return 0;//coingo.vip
    if (nHeight == 1) return COIN * 1000*10000*10000;//coingo.vip
...
{
    LOCK(cs_main);
    int32_t nVersion = VersionBitsTopBits(pindexPrev != NULL ? (pindexPrev->nHeight + 1) : 0);
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    // Start enforcing nonceOf opcode
    if (pindex->nHeight >= consensusparams.NonceOfHeight) {
        flags |= SCRIPT_VERIFY_NONCEOF;
    }
